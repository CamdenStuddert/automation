...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name(`q`)).sendKeys("How do Chinese people pronounce beijing?", Key.RETURN);
  await driver.wait(until.titleIs("How do Chinese people pronounce beijing? - Google Search"), 1000);
  ...
  await driver.findElement(By.name(`q`)).sendKeys("Beijing", Key.RETURN)
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs(`Beijing - Google Search`), 1000);
});
